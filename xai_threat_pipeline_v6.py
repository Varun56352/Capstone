#!/usr/bin/env python3
"""End-to-end XAI cyber threat pipeline for UNSW-NB15 + TON_IoT (v6 - SMOTENC + Tabular Metrics).

Implements:
1) Common attack mapping
2) Unified preprocessing with RobustScaler
3) Advanced rate-based Feature Engineering
4) SMOTENC to handle categorical data oversampling without destroying features
5) Model comparison with heavily tuned baseline architectures
6) XAI-guided feature selection (SHAP + LIME)
7) Within-dataset and Cross-dataset evaluations
8) Automated Tabular Metrics & XAI Executive Summary
"""
...
UNSW_MAP = {
    "normal": "Normal", "backdoor": "R2L/BruteForce", "analysis": "Recon/Probe",
    "fuzzers": "Recon/Probe", "reconnaissance": "Recon/Probe", "shellcode": "R2L/BruteForce",
    "dos": "DoS/DDoS", "exploits": "R2L/BruteForce", "worms": "Other", "generic": "DoS/DDoS",
}

TON_MAP = {
    "normal": "Normal", "ddos": "DoS/DDoS", "dos": "DoS/DDoS", "scanning": "Recon/Probe",
    "reconnaissance": "Recon/Probe", "password": "R2L/BruteForce", "xss": "R2L/BruteForce",
    "injection": "R2L/BruteForce", "mitm": "Other", "backdoor": "R2L/BruteForce", "ransomware": "Other",
}
...
UNSW_TO_COMMON = {"sport": "src_port", "dsport": "dst_port", "dur": "duration", "sbytes": "src_bytes", "dbytes": "dst_bytes", "state": "conn_state", "Spkts": "src_pkts", "Dpkts": "dst_pkts"}
...
def normalize_label(value: object) -> str: return str(value).strip().lower()
...
_CONN_STATE_MAP = {}
for s in ("FIN", "CON", "CLO"): _CONN_STATE_MAP[s.lower()] = "established"
for s in ("REQ", "RST", "no"): _CONN_STATE_MAP[s.lower()] = "rejected"
for s in ("ACC", "INT", "PAR", "URH", "URN", "ECO"): _CONN_STATE_MAP[s.lower()] = "attempt"
for s in ("SF", "S1", "S2", "S3"): _CONN_STATE_MAP[s.lower()] = "established"
for s in ("REJ", "RSTO", "RSTR", "RSTOS0", "RSTRH"): _CONN_STATE_MAP[s.lower()] = "rejected"
for s in ("S0", "SH", "SHR", "OTH"): _CONN_STATE_MAP[s.lower()] = "attempt"

def normalize_conn_state(series: pd.Series) -> pd.Series:
    return series.astype(str).str.strip().str.lower().map(lambda v: _CONN_STATE_MAP.get(v, "other"))
...
def engineer_features(X: pd.DataFrame) -> pd.DataFrame:
    ...
    if "src_bytes" in X.columns and "dst_bytes" in X.columns:
        sb = pd.to_numeric(X["src_bytes"], errors="coerce").fillna(0)
        db = pd.to_numeric(X["dst_bytes"], errors="coerce").fillna(0)
        X["byte_ratio"] = sb / (sb + db).replace(0, 1)
    if "src_pkts" in X.columns and "dst_pkts" in X.columns:
        sp = pd.to_numeric(X["src_pkts"], errors="coerce").fillna(0)
        dp = pd.to_numeric(X["dst_pkts"], errors="coerce").fillna(0)
        X["pkt_ratio"] = sp / (sp + dp).replace(0, 1)
    if "duration" in X.columns:
        dur = pd.to_numeric(X["duration"], errors="coerce").fillna(0).replace(0, 0.0001)
        if "src_bytes" in X.columns: X["src_bytes_per_sec"] = pd.to_numeric(X["src_bytes"], errors="coerce").fillna(0) / dur
        if "dst_bytes" in X.columns: X["dst_bytes_per_sec"] = pd.to_numeric(X["dst_bytes"], errors="coerce").fillna(0) / dur
        if "src_pkts" in X.columns: X["src_pkts_per_sec"] = pd.to_numeric(X["src_pkts"], errors="coerce").fillna(0) / dur
        if "dst_pkts" in X.columns: X["dst_pkts_per_sec"] = pd.to_numeric(X["dst_pkts"], errors="coerce").fillna(0) / dur
...
def load_dataset(path: Path, label_col: str, dataset_name: str) -> DatasetBundle:
    ...
    try: df = pd.read_csv(path, low_memory=False, encoding="latin-1")
    except: df = pd.read_csv(path, header=None, low_memory=False)
    
    if dataset_name == "UNSW-NB15" and len(df.columns) == len(UNSW_COLUMNS):
        df.columns = UNSW_COLUMNS
    if label_col not in df.columns: label_col = df.columns[-1]

    y_raw = df[label_col].copy()
    if dataset_name == "UNSW-NB15" and label_col == "attack_cat": y_raw = y_raw.fillna("Normal").replace("", "Normal")

    mask = y_raw.notna() & (y_raw.astype(str).str.strip() != "")
    df, y_raw = df.loc[mask], y_raw.loc[mask]
    ...
    if dataset_name == "UNSW-NB15": X = X.rename(columns=UNSW_TO_COMMON)
    if "conn_state" in X.columns: X["conn_state"] = normalize_conn_state(X["conn_state"])
...
class CNNWrapper(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        if len(self.classes_) > 2: indices = np.argmax(probs, axis=1)
        else: indices = (probs[:, 1] >= 0.5).astype(int)
        return self.classes_[indices]
        
    def predict_proba(self, X):
        X_reshaped = X.reshape((X.shape[0], X.shape[1], 1))
        preds = self.model.predict(X_reshaped, verbose=0)
        if len(self.classes_) == 2: return np.hstack([1 - preds, preds])
        return preds

def get_models() -> Dict[str, object]:
    return {
        "RandomForest": RandomForestClassifier(n_estimators=100, max_depth=None, min_samples_split=5, random_state=42, n_jobs=-1),
...
def run_evaluation(X_train, y_train, X_test, y_test, feature_names, cat_cols_len, eval_name, out_dir, max_rows=15000):
    # Subset before SMOTENC for computational efficiency
    if len(X_train) > max_rows:
        rng = np.random.default_rng(42)
        idx = rng.choice(len(X_train), size=max_rows, replace=False)
        X_train_sub = X_train[idx]
        y_train_sub = y_train.iloc[idx] if isinstance(y_train, pd.Series) else y_train[idx]
    else:
        X_train_sub, y_train_sub = X_train, y_train

    num_cols_len = len(feature_names) - cat_cols_len
    cat_indices = list(range(num_cols_len, len(feature_names)))
    
    print(f"    [{eval_name}] Applying SMOTENC for oversampling on {len(X_train_sub)} samples...")
    try:
        if len(cat_indices) > 0:
            smote = SMOTENC(categorical_features=cat_indices, random_state=42, n_jobs=-1)
        else:
            smote = SMOTE(random_state=42, n_jobs=-1)
        X_train_bal, y_train_bal = smote.fit_resample(X_train_sub, y_train_sub)
    except Exception as e:
        print(f"      [SMOTENC] Warning, failed. Using original data. Error: {e}")
        X_train_bal, y_train_bal = X_train_sub, y_train_sub
    ...
    for name, model in base_models.items():
        print(f"    [{eval_name}] Training {name} on {len(X_train_bal)} balanced samples...")
...
def generate_xai_summary_report(metrics_df: pd.DataFrame, out_dir: Path):
    """Generates an automated Markdown report with tabular metrics."""
    ...
    with open(report_path, "w") as f:
        f.write("# XAI Cybersecurity Threat Detection - Executive Summary (v6)\n\n")
        
        # TABULAR METRICS SECTION
        f.write("## 1. Complete Evaluation Metrics Table\n")
        f.write("Below is the tabular summary of all models tested within and across datasets:\n\n")
        
        f.write("| Train Dataset | Test Dataset | Model Pipeline | Accuracy | F1-Score (Weighted) |\n")
        f.write("| :--- | :--- | :--- | :--- | :--- |\n")
        ...
        for _, row in metrics_df.iterrows():
            ...
            acc = row['accuracy']
            f1 = row['f1_weighted']
            # Make cross dataset transfers bold to highlight them
            if train != test:
                f.write(f"| **{train}** | **{test}** | **{model}** | **{acc:.4f}** | **{f1:.4f}** |\n")
            else:
                f.write(f"| {train} | {test} | {model} | {acc:.4f} | {f1:.4f} |\n")
        ...
        if not within_ton.empty and not cross_ton_unsw.empty:
            ...
            f.write(f"- When trained and evaluated on the **same** dataset (TON_IoT), the best model ({best_within['model']}) achieves an F1-score of **{best_within['f1_weighted']:.4f}**.\n")
            f.write(f"- When transferring that training to a **different** dataset (UNSW-NB15), the best model ({best_cross['model']}) drops to an F1-score of **{best_cross['f1_weighted']:.4f}**.\n\n")
        
        f.write("## 3. The Power of XAI Feature Selection\n")
        f.write("By utilizing `SMOTENC` to properly oversample data alongside XAI Feature Selection, the models are forced to rely on fundamentally true properties of attacks rather than dataset-specific imbalances.\n\n")
        ...
        f.write(f"- Using SHAP-selected features improved generalization in **{improved_cases}** out of the cross-dataset instances compared to using all raw features.\n\n")
        f.write("## 4. Visual Evidence\n")
        f.write("Check the `outputs_v6/` directory for `.png` files showing **SHAP Summary Plots**.\n")
...
def main() -> None:
    ...
    bin_unsw_unsw, bin_u_u_dom = run_evaluation(X_train_u, y_train_u, X_test_u, y_test_u, feature_names, cat_cols_len, "UNSW->UNSW Binary", args.out_dir)
    ...
    bin_ton_ton, bin_t_t_dom = run_evaluation(X_train_t, y_train_t, X_test_t, y_test_t, feature_names, cat_cols_len, "TON->TON Binary", args.out_dir)
    ...
    bin_unsw_ton, bin_unsw_ton_dom = run_evaluation(X_unsw, y_unsw_bin, X_ton, y_ton_bin, feature_names, cat_cols_len, "UNSW->TON Binary", args.out_dir)
    ...
    bin_ton_unsw, bin_ton_unsw_dom = run_evaluation(X_ton, y_ton_bin, X_unsw, y_unsw_bin, feature_names, cat_cols_len, "TON->UNSW Binary", args.out_dir)
    ...
    with open(args.out_dir / "dominant_features_binary.json", "w") as f:
        json.dump({
            "UNSW_UNSW": bin_u_u_dom, "TON_TON": bin_t_t_dom,
            "UNSW_TON": bin_unsw_ton_dom, "TON_UNSW": bin_ton_unsw_dom
        }, f, indent=2)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=Path("outputs_v6"))
    return p.parse_args()
